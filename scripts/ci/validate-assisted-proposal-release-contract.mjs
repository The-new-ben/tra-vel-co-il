...
if (!themeStyle.includes('Version: 1.33.0') || !themeFunctions.includes("define( 'TRA_VEL_V2_VERSION', '1.33.0' )")) {
  failures.push('Theme header and runtime version must both identify release 1.33.0.');
}

const dependency = requirements.dependencies?.find(item => item.id === 'tra-vel-agent-core');
if (requirements.theme?.version !== '1.33.0' || dependency?.min_version !== '0.7.0') {
  failures.push('Theme release requirements must bind theme 1.33.0 to Agent Core 0.7.0 or newer.');
}
