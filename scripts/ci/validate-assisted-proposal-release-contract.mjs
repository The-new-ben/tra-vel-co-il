...
if (!/\* Version: 0\.7\.0/.test(main) || !/TRA_VEL_AGENT_VERSION', '0\.7\.0'/.test(main)) {
  failures.push('Agent Core header and runtime version must both identify release 0.7.0.');
}
...
if (!readme.includes('Stable tag: 0.7.0') || !readme.includes('= 0.7.0 =')) {
  failures.push('Agent Core readme must identify release 0.7.0.');
}
if (!themeStyle.includes('Version: 1.20.0') || !themeFunctions.includes("define( 'TRA_VEL_V2_VERSION', '1.20.0' )")) {
  failures.push('Theme header and runtime version must both identify release 1.20.0.');
}

const dependency = requirements.dependencies?.find(item => item.id === 'tra-vel-agent-core');
if (requirements.theme?.version !== '1.20.0' || dependency?.min_version !== '0.7.0') {
  failures.push('Theme release requirements must bind theme 1.20.0 to Agent Core 0.7.0 or newer.');
}
