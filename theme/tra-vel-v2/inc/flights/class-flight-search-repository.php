<?php
/**
 * Cached, stale-safe flight search repository.
 *
 * @package TraVelV2
 */
...
class Tra_Vel_V2_Flight_Search_Repository {
	const CACHE_TTL = 300;
	const STALE_TTL = 1800;
	const LOCK_TTL  = 20;

	/** @var Tra_Vel_V2_Flight_Search_Registry */
	private $registry;

	public function __construct( $registry = null ) {
		$this->registry = $registry instanceof Tra_Vel_V2_Flight_Search_Registry ? $registry : new Tra_Vel_V2_Flight_Search_Registry();
	}
	...
	public function search( $query, $force = false ) {
		...
		try {
			...
			$resolved = new WP_Error( 'tra_vel_flight_search_exception', 'Flight search failed safely.' );
		}
		...
		if ( ! empty( $resolved['degraded'] ) && $this->is_result( $stale ) && 'live' === $stale['data']['data_mode'] ) {
			...
			$stale['runtime']['fallback_error']  = 'flight_supplier_degraded';
			return $this->with_state( $stale, 'stale_error' );
		}
	...
	public function purge() {
		...
		update_option( 'tra_vel_v2_flight_cache_generation', $generation + 1, false );
		return $generation + 1;
	}
