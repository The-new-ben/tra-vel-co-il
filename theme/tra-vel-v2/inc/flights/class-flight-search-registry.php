<?php
/**
 * Flight adapter registry with explicit demo fallback.
 *
 * @package TraVelV2
 */
...
class Tra_Vel_V2_Flight_Search_Registry {
	...
	public function __construct() {
		$adapters = apply_filters( 'tra_vel_v2_flight_search_adapters', array() );
		if ( is_array( $adapters ) ) {
			foreach ( $adapters as $adapter ) {
				$this->register( $adapter );
			}
		}
		$this->register( new Tra_Vel_V2_Demo_Flight_Search_Adapter() );
	}

	public function register( $adapter ) {
		if ( ! $adapter instanceof Tra_Vel_V2_Flight_Search_Adapter ) {
			return false;
		}
	...
	public function resolve( $query ) {
		...
		if ( ! $demo_adapter ) {
			return new WP_Error( 'tra_vel_flights_unavailable', 'No flight search adapter is available.', array( 'status' => 503 ) );
		}
