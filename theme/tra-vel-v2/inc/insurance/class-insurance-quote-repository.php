<?php
/**
 * Cached, stale-safe insurance quote repository.
 *
 * @package TraVelV2
 */
...
class Tra_Vel_V2_Insurance_Quote_Repository {
	const CACHE_TTL = 300;
	const STALE_TTL = 1800;
	const LOCK_TTL  = 20;

	/** @var Tra_Vel_V2_Insurance_Quote_Registry */
	private $registry;

	public function __construct( $registry = null ) {
		$this->registry = $registry instanceof Tra_Vel_V2_Insurance_Quote_Registry ? $registry : new Tra_Vel_V2_Insurance_Quote_Registry();
	}

	public function quote( $query, $force = false, $cache_enabled = true ) {
		if ( ! $cache_enabled ) {
			try {
				$resolved = $this->registry->resolve( $query );
			} catch ( Throwable $error ) {
				$resolved = new WP_Error( 'tra_vel_insurance_quote_exception', 'Insurance comparison failed safely.' );
			}
			...
			return array(
				'data' => $resolved['data'],
				'runtime' => array(
					'generated_at' => gmdate( 'c' ), 'cache_state' => 'bypass_sensitive', 'cache_ttl' => 0, 'stale_ttl' => 0,
					'degraded' => ! empty( $resolved['degraded'] ), 'failed_adapters' => $resolved['failed_adapters'], 'adapters' => $resolved['reports'],
				),
			);
		}
		...
		if ( ! empty( $resolved['degraded'] ) && $this->is_result( $stale ) && 'live' === $stale['data']['data_mode'] ) {
			...
			$stale['runtime']['fallback_error']  = 'insurance_supplier_degraded';
			return $this->with_state( $stale, 'stale_error' );
		}
	...
	public function purge() {
		$generation = (int) get_option( 'tra_vel_v2_insurance_cache_generation', 1 );
		update_option( 'tra_vel_v2_insurance_cache_generation', $generation + 1, false );
		return $generation + 1;
	}

	private function cache_key( $query ) {
		$generation = (int) get_option( 'tra_vel_v2_insurance_cache_generation', 1 );
		return 'tv2_insurance_' . md5( wp_json_encode( array( $generation, $this->registry->get_cache_signature(), $query ) ) );
	}
