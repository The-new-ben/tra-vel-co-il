<?php
/**
 * Package adapter registry with explicit demo fallback.
 *
 * @package TraVelV2
 */
...
class Tra_Vel_V2_Trip_Package_Registry {
	...
	public function __construct() {
		$adapters = apply_filters( 'tra_vel_v2_trip_package_adapters', array() );
		if ( is_array( $adapters ) ) {
			foreach ( $adapters as $adapter ) {
				$this->register( $adapter );
			}
		}
		$this->register( new Tra_Vel_V2_Demo_Trip_Package_Adapter() );
	}

	public function register( $adapter ) {
		if ( ! $adapter instanceof Tra_Vel_V2_Trip_Package_Adapter ) {
			return false;
		}
	...
	public function resolve( $query ) {
		...
		foreach ( $this->adapters as $id => $adapter ) {
			...
			$report = array(
				'id' => $id, 'mode' => $adapter->get_mode(), 'configured' => (bool) $adapter->is_configured(),
				'healthy' => false, 'used' => false, 'error_code' => null,
			);
		...
		if ( ! $demo_adapter ) {
			return new WP_Error( 'tra_vel_packages_unavailable', 'No package adapter is available.', array( 'status' => 503 ) );
		}
		...
		$reports[ $demo_id ] = array(
			'id' => $demo_id, 'mode' => 'demo', 'configured' => true, 'healthy' => true, 'used' => true, 'error_code' => null,
		);
