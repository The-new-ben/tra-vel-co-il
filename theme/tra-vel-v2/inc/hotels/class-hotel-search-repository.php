<?php
/**
 * Cached, stale-safe hotel search repository.
 *
 * @package TraVelV2
 */
...
class Tra_Vel_V2_Hotel_Search_Repository {
	const CACHE_TTL = 300;
	const STALE_TTL = 1800;
	const LOCK_TTL  = 20;

	/** @var Tra_Vel_V2_Hotel_Search_Registry */
	private $registry;

	public function __construct( $registry = null ) {
		$this->registry = $registry instanceof Tra_Vel_V2_Hotel_Search_Registry ? $registry : new Tra_Vel_V2_Hotel_Search_Registry();
	}
	...
	public function search( $query, $force = false ) {
		...
		try {
			...
			$resolved = new WP_Error( 'tra_vel_hotel_search_exception', 'Hotel search failed safely.' );
		}
		...
		if ( ! empty( $resolved['degraded'] ) && $this->is_result( $stale ) && 'live' === $stale['data']['data_mode'] ) {
			...
			$stale['runtime']['fallback_error']  = 'hotel_supplier_degraded';
			return $this->with_state( $stale, 'stale_error' );
		}
		...
		$result   = array(
			...
			'runtime' => array(
				...
				'degraded'        => $degraded,
				'failed_adapters' => $resolved['failed_adapters'],
	...
	public function purge() {
		$generation = (int) get_option( 'tra_vel_v2_hotel_cache_generation', 1 );
		update_option( 'tra_vel_v2_hotel_cache_generation', $generation + 1, false );
		return $generation + 1;
	}

	private function cache_key( $query ) {
		$generation = (int) get_option( 'tra_vel_v2_hotel_cache_generation', 1 );
		return 'tv2_hotels_' . md5( wp_json_encode( array( $generation, $this->registry->get_cache_signature(), $query ) ) );
	}
