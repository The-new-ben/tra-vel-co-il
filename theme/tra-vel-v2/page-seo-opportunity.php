...
$globe_pin_price     = $globe_point ? tra_vel_v2_get_destination_price( $map_state ) : null;
$globe_pin_price_tag = tra_vel_v2_found_price_pin_label( $globe_pin_price );
	...
	<header class="directory-hero">
		<div class="directory-hero-grid page-width">
			...
			<div class="compact-map" aria-label="<?php echo esc_attr( $globe_label ); ?>">
				...
				<div class="globe globe-webgl" data-globe-3d data-origin-latitude="32.0005" data-origin-longitude="34.8708" data-texture="<?php echo esc_url( tra_vel_v2_asset_uri( 'images/earth-blue-marble-2048.jpg' ) ); ?>" data-texture-hd="<?php echo esc_url( tra_vel_v2_asset_uri( 'images/earth-blue-marble-4096.jpg' ) ); ?>" data-book-cta-label="<?php esc_attr_e( 'בחרו והמשיכו להזמנה', 'tra-vel-v2' ); ?>" data-book-scope-note="<?php echo esc_attr( tra_vel_v2_found_price_scope_note() ); ?>" tabindex="0" role="group" aria-label="<?php echo esc_attr( $globe_label ); ?>"><canvas data-globe-canvas aria-hidden="true"></canvas><noscript><img class="globe-noscript-image" src="<?php echo esc_url( tra_vel_v2_asset_uri( 'images/earth-blue-marble-2048.jpg' ) ); ?>" alt="<?php esc_attr_e( 'מפת עולם סטטית', 'tra-vel-v2' ); ?>"></noscript><svg class="globe-route-layer" data-globe-routes width="100%" height="100%" aria-hidden="true"><path data-globe-route></path></svg><span class="origin-point" data-globe-origin aria-label="<?php esc_attr_e( 'תל אביב, נקודת מוצא', 'tra-vel-v2' ); ?>"></span><?php if ( $globe_point ) : ?><a class="price-pin is-active" data-destination="<?php echo esc_attr( $map_state ); ?>" data-latitude="<?php echo esc_attr( $globe_point['latitude'] ); ?>" data-longitude="<?php echo esc_attr( $globe_point['longitude'] ); ?>"<?php echo $globe_pin_price_tag ? ' data-found-price="' . esc_attr( $globe_pin_price_tag ) . '"' : ''; ?><?php echo ( $globe_pin_price_tag && ! empty( $globe_pin_price['deep_link'] ) ) ? ' data-book-link="' . esc_url( $globe_pin_price['deep_link'] ) . '"' : ''; ?> aria-label="<?php echo esc_attr( $entry['primaryIntent'] ); ?>" aria-current="location" href="<?php echo esc_url( $map_url ); ?>"><?php echo esc_html( $map_marker ); ?></a><?php endif; ?><span class="screen-reader-text" data-globe-live aria-live="polite"></span><?php tra_vel_v2_premium_planet_control(); ?></div>
